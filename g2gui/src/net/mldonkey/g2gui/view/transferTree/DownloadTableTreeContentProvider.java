 ...
 *
 * @version $Id: DownloadTableTreeContentProvider.java,v 1.25 2003/09/18 12:12:23 lemmster Exp $
 *
 */
public class DownloadTableTreeContentProvider implements ITreeContentProvider, Observer, TreeListener {
    private static final int CHUNKS_COLUMN = DownloadTableTreeViewer.getChunksColumn();
    ...
    private static final String[] allProperties = { FileInfo.CHANGED_RATE, FileInfo.CHANGED_DOWNLOADED,
    												  FileInfo.CHANGED_PERCENT, FileInfo.CHANGED_AVAIL,
    												  FileInfo.CHANGED_ETA, FileInfo.CHANGED_LAST };
            ...
            if ( arg instanceof String[] ) {
                ...
                for ( int i = 0; ( list != null ) && ( i < list.size() ); i++ ) {
                    ...
                    if ( ( treeClientInfo.getFileInfo().getId() == t.getFileInfo().getId() )
                             && ( 
                                 treeClientInfo.getClientInfo().getClientid() == t.getClientInfo()
                                                                                          .getClientid()
                              ) ) {
	 ...
	 * DOCUMENT ME!
	 * 
	 * @param tableTreeItem DOCUMENT ME!
	 */
    ...
    public void openEditor( TableTreeItem tableTreeItem ) {
        ...
        chunkCanvas =
            new ChunkCanvas( tableTreeViewer.getTableTree().getTable(), SWT.NO_BACKGROUND, clientInfo,
                             fileInfo, null );
